...
module StatsHelper
	...
	def get_data(row_count=25, order_by="AVG", desc=true)		
		players = Rails.cache.fetch("xml_flattened_data", :expires_in => 1.hour) { fetch_data }
		#
		order_by.downcase!
		...
		players.sort_by! {|player| 
			case order_by
				when :home_runs,:rbi,:runs,:avg,:sbp,:ops then player[order_by].to_f
